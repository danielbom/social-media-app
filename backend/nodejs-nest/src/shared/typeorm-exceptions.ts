...
type ExceptionHandler<T> = TypeormExceptionsT<(exception: QueryFailedError) => T>

export const TypeormExceptionsMatch: ExceptionHandler<RegExpMatchArray | null> = {
  TableDoesNotExists: {
    mysql(exception) {
      return exception.message.match(/Table '(?<table>[\d\w]+)' doesn't exist/)
    },
    postgres(exception) {
      return exception.message.match(/relation "(?<table>[\d\w]+)" does not exist/)
    },
  },
  ViolatesForeignKeyConstraint: {
    mysql(exception) {
  ...
  Object.entries(TypeormExceptionsMatch).map(([exceptionName, values]) => {
    const predicates = Object.fromEntries(
      Object.entries(values).map(([database, matcher]) => {
        ...
        if (code) return [database, (exception: any) => exception.code === code]
