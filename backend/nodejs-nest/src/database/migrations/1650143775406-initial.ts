  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      CREATE TABLE \`comment\` (
        ...
        \`likes\` int NOT NULL,
        \`author_id\` varchar(255) NOT NULL,
        \`post_parent_id\` varchar(255) NOT NULL,
        \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
        \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
        \`deleted_at\` datetime(6) NULL,
    ...
    `);
    await queryRunner.query(`
      ALTER TABLE \`comment\` ADD CONSTRAINT \`FK_137a2718c6c677cb5e675c4e283\` FOREIGN KEY (\`post_parent_id\`) REFERENCES \`post\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION
    `);
    await queryRunner.query(`
      ALTER TABLE \`comment\` ADD CONSTRAINT \`FK_6a49447281e4d5525273b497a16\` FOREIGN KEY (\`comment_parent_id\`) REFERENCES \`comment\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION
    `);
  }
