...
export class AdminJSOptionsFactory {
  ...
  async create(): Promise<AdminModuleOptions> {
    return {
            ...
            options: {
              new: {
                before: async (request: RecordActionResponse) => {
                  request.payload.password = await this.hashService.hash(request.payload.password)
                },
              },
            },
          },
      ...
      auth: {
        authenticate: (username, password) => this.autenticate(username, password),
  ...
  private async autenticate(username: string, password: string): Promise<CurrentAdmin | null> {
    try {
