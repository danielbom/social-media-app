...
function middlewareMustBeCalled(middlewareCount: Partial<Record<keyof MiddlewareSpy, number>>) {
...
describe('Middleware', () => {
  ...
  it('/auth/login (POST)', async () => {
    await request(app.getHttpServer()).post('/auth/login').send({ username: '123', password: '12345678' })
  ...
  it('/comments (GET)', async () => {
    await request(app.getHttpServer()).get('/comments').set('Authorization', authorization).expect(HttpStatus.OK)
  ...
  it('/posts (GET)', async () => {
    await request(app.getHttpServer()).get('/posts').set('Authorization', authorization).expect(HttpStatus.OK)
